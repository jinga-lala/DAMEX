...
class MoEMlp(nn.Module):
    def __init__(self, in_features, hidden_features, num_local_experts, top_value, capacity_factor=1.25,
                 cosine_router=False, normalize_gate=True, use_bpr=True, is_gshard_loss=True,
                 gate_noise=1.0, cosine_router_dim=256, cosine_router_init_t=0.5, moe_drop=0.2, init_std=0.02,
                 mlp_fc2_bias=True, damex_loss=True):
        super().__init__()
        ...
        _gate_type = {'type': 'cosine_top' if cosine_router else 'top',
                      ...
                      'gate_noise': gate_noise, 'fp32_gate': False}
        if cosine_router:
        ...
        self._moe_layer = tutel_moe.moe_layer(
            ...
            damex_loss=damex_loss,
            # a2a_ffn_overlap_degree = 1,
            # parallel_type = 'auto',
            # use_2dh=False,
        )
        self.num_global_experts = self._moe_layer.num_global_experts

        if not self.mlp_fc2_bias:
            self._moe_layer.experts.batched_fc2_bias.requires_grad = False
        # self._moe_layer.experts.batched_fc1_w.requires_grad=False
        # self._moe_layer.experts.batched_fc1_bias.requires_grad=False
        # self._moe_layer.experts.batched_fc2_w.requires_grad=False
        
    def forward(self, x):
        # breakpoint()
        x, moe_loss_fn = self._moe_layer(x, inequivalent_tokens=True)
        # return x, x.l_aux
        return x, x.l_aux, moe_loss_fn
        ...
        if deformable_decoder:
            if not use_moe:
                ...
                for i in range(num_decoder_layers):
                    module_seq = self.module_seq[i]
                    decoder_layer.append(DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                    dropout, activation,
                                                    num_feature_levels, nhead, dec_n_points, use_deformable_box_attn=use_deformable_box_attn, box_attn_type=box_attn_type,
                                                    key_aware_type=key_aware_type,
                                                    decoder_sa_type=decoder_sa_type,
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 ...
                 top_value = 1,
                 cosine_router=False,
                 normalize_gate=False, use_bpr=True, 
                 is_gshard_loss=False, # from swin-moe configs
                 gate_noise=1.0,
                 cosine_router_dim=256, cosine_router_init_t=0.5, moe_drop=0.0,
                 mlp_fc2_bias=False, # from swin-moe configs
                 init_std=0.005, # from swin-moe configs
                 drop_path=0.2, #not applied on FFN 
                 damex_loss=True,
                 ):
        super().__init__()
        self.module_seq = module_seq
        assert sorted(module_seq) == ['ca', 'ffn', 'sa'] or sorted(module_seq) == ['ca', 'ffn_moe', 'sa']
        # cross attention
        if use_deformable_box_attn:
        ...
        if 'ffn' in module_seq:
        ...
        elif 'ffn_moe'  in module_seq:
            self._moe_layer = MoEMlp(in_features=d_model,
            ...
            _gate_type = {'type': 'top',
                        'k': top_value, 'capacity_factor': capacity_factor,
                        'gate_noise': 1.0, 'fp32_gate': False}
            self.activation = _get_activation_fn(activation, d_model=d_ffn, batch_dim=1)
            # self._moe_layer = tutel_moe.moe_layer(
            self._moe_layer = moe_layer(
                gate_type=_gate_type,
                model_dim=d_model,
                experts={'type': 'ffn', 'count_per_node': num_local_experts, 'hidden_size_per_expert': d_ffn,
                        # 'activation_fn': lambda x: self._dropout(F.gelu(x))
    ...
    def forward_ffn_moe(self, tgt):
        '''
        tgt = self._moe_layer(tgt)
        self.loss_moe = tgt.l_aux
        '''
        shortcut = tgt
        x = self.norm3(tgt)
        # tgt_x, self.loss_moe = self._moe_layer(x)#, inequivalent_tokens=True)
        tgt_x, self.loss_moe, self.moe_loss_fn = self._moe_layer(x)#, inequivalent_tokens=True)
        tgt_x = shortcut + self.drop_path(tgt_x)
        # for n,p in self._moe_layer.named_parameters():  print(n, p[:,0,0]); break #if len(p.shape) > 2 else break  #print(n, p[:,0])
        # print(self.loss_moe, dist.get_rank())
        # print('-----------------')
        return tgt_x
    ...
    def forward(self,
        ...
        for funcname in self.module_seq:
            ...
            elif funcname == 'ffn_moe':
                tgt = self.forward_ffn_moe(tgt)
            elif funcname == 'ca':
...
def build_deformable_transformer(args):
    ...
    # check if use_moe argument exist in args variable
    use_moe = False
    try:
    ...
    if use_moe:
        normalize_gate= args.moe_normalize_gate
        is_gshard_loss= args.moe_is_gshard_loss
        mlp_fc2_bias= args.moe_mlp_fc2_bias
        drop_path= args.moe_drop_path
        init_std= args.moe_init_std
