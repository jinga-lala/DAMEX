...
class _VOCBase(VisionDataset):
    ...
    def __init__(
        ...
        self.year = verify_str_arg(year, "year", valid_values=[str(yr) for yr in range(2007, 2033)])
        ...
        for enum, img in enumerate(images_copy):
            if not os.path.exists(img):
                # remove img
                print(img)
                self.images.remove(img)
                file_names.remove(img.split('/')[-1].split('.')[0])
