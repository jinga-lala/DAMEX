...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        #     dataset_idx = torch.tensor(dataset_idx, dtype=torch.long, device=device)
        ...
        if args.use_wandb and utils.is_main_process():
            if hasattr(args, 'use_moe') and (args.use_moe):
                wandb.log({'train_loss': loss_value, 'loss_moe': loss_dict_reduced_scaled['loss_moe']}, step=epoch*len(data_loader)+_cnt)
            else:
                wandb.log({'train_loss': loss_value}, step=epoch*len(data_loader)+_cnt)
